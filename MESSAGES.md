# MESSAGES.md — Mensajes predefinidos Aero-Freez

Array de 75 mensajes en espanol, sin emojis, sin caracteres especiales,
con tildes correctas. Se elige uno al azar cada vez que se escanea el QR.
...
- Copiar este array exactamente en el backend y en la mini web
- En el backend: seleccionar con `mensajes[Math.floor(Math.random() * mensajes.length)]`
- En Flutter: usar `mensajes[Random().nextInt(mensajes.length)]`
- No modificar los textos, respetar tildes y puntuacion
